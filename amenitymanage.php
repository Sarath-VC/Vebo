        ...
        <body class="  ">
            <div class="bg-dark dk" id="wrap">
                <div id="top">
                    ...
                    </nav>                       <header class="head"><!-- /.search-bar -->
                            <div class="main-bar">
                               <h3>
              <i class="col-lg-5"></i>   &nbsp;
            AMENITIES MANAGEMENT </h3>
                      </div>
                            <!-- /.main-bar -->
                        </header>
                        <!-- /.head -->
...
<div class="row">
        ...
        <div class="box">
            <header>
                <ul class="nav nav-tabs pull-right">
                    <li class="active" name="manage"><a href="#markdown" data-toggle="tab">MANAGE</a></li>
	...
	<div class="box">
     <header>
            <div class="icons"><i class="">AMENITIES</i></div>
		...
		<table class="table table-condensed table-hovered sortableTable">
		    <thead>
			<tr>
			    <th>AMENITY ID<i class="fa sort"></i></th>
			...
			<?php
               ...
               $db=mysql_select_db("vebo",$con);
               if(!$db)
               {
               	die('Could not select db : '.mysql_error());
               }
                  $sql="Select * from aminities join department on department.dept_id=aminities.dept_id and aflag=1";
...
echo "<tr><form method='GET'><td>".$row['a_id']."</td><td>".$row['a_name']."</td><td>".$row['a_type']."</td><td>";
$ve=$row['v_id'];
$sqlve="Select * from venue where v_id=$ve and vflag=1";
                    $resultve=mysql_query($sqlve,$con);
                    while($rowve=mysql_fetch_assoc($resultve)){

                       echo $rowve['v_name']; } echo"</td><td>".$row['dept_name']."</td><td><img alt='No Picture' src='data:image/jpeg; base64,". base64_encode($row['a_image'])."' width='100' height='100'/></td><td>";
            ?>    <a href="vieamenity.php?aid=<?php echo $row['a_id']; ?>"><span class="glyphicon glyphicon-eye-open"></span></a>&nbsp;&nbsp;<a href="editamenity.php?aid=<?php echo $row['a_id']; ?>"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;<a href="deleteamenity.php?aid=<?php echo $row['a_id']; ?>">
...
<div class="col-lg-12">
    <div class="box dark">
    <header>
                    <div class="icons"><i class="glyphicon glyphicon-blackboard"> AMENITY REGISTRATION</i></div>
                        <h5><!-- .toolbar -->            </h5>
        ...
        <div id="" class="body">
            ...
            <div class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-4">
                        <input type="text" required id="text1" placeholder="" class="form-control" name="aname">
                    </div>
                </div>
                  <!-- /.form-group -->
 <div class="form-group">
    ...
    <div class="col-lg-4">
        <select required class="form-control" name="atype">
 ...
 <div class="form-group">
    ...
    <div class="col-lg-4">
        <select required class="form-control" name="adep" id="adep" onchange="showUser(this.value)">
            <option value="" required pattern="select"> select </option>
            <?php 
            ...
            <option value="" required pattern="select"> select </option>
                ...
                <!-- /.form-group -->
                 <div class="form-group">
                    ...
                    <div class="col-lg-4">
                        <input type="file" value="" accept="image/*" class="form-control" name="aimg">
                    </div>
                </div>
                <!-- /.form-group -->
...
<button class="btn btn-primary "  type="submit" name="btnp">PROCEED</button>
<button class="btn btn-primary pull-right " type="submit" name="btnd" onclick="disca()">DISCARD</button>
            ...
            <?php
               ...
               $conam=addslashes(file_get_contents($_FILES['aimg']['tmp_name']));
               	$sql="insert into aminities(a_name,a_type,a_image,dept_id,aflag,v_id)values('$_POST[aname]','$_POST[atype]','{$conam}','$_POST[adep]',1,'$_POST[avenue]')";
                $result=mysql_query($sql,$con);
                  //while($row=mysql_fetch_assoc($result))
                  if(!$result)
                 { die('Error'.mysql_error($con)); }
...
<script type="text/javascript">alert("Successfully Inserted");</script>
          ...
          mysql_close($con);     
            ...
            ?>  
				<script> function disca(){ window.location.assign("http://localhost/vebo/amenitymanage.php"); }</script> 	
                    </div>
                </div>
            </div>
        </div>
