...
<html lang="en">
	...
	<div class="col-md-12 col-sm-12 col-xs-12" style="background-color:#e2e4ec;">
        <div class="x_panel">
                  <div class="x_title" style="background-color:#2e2e33; color:white; font-family:Segoe, 'Segoe UI', 'DejaVu Sans', 'Trebuchet MS', Verdana, sans-serif">
					...
					<li><a href="cancellations.php" class="close-link"><i class="glyphicon glyphicon-trash"></i></a></li>
                    </ul>
		...
		<table class="table table-condensed table-hovered sortableTable">
		    <thead>
			<tr>
			    <th>Request ID<i class="fa sort"></i></th>
                <th>EVENTNAME<i class="fa sort"></i></th>
  ...
  $row2=mysql_fetch_assoc($res2);
    ...
    $sqlv="select * from cell where cell_id=$c and cell_status='Requested'";
       ...
       echo $rowv1['v_name'];
        echo "</td><td>".$row['start_date']." To ".$row['finish_date']."</td><td>".$row['purpose']."</td><td>";
            ...
            <input type="hidden" name="rfno" value="<?php echo $row['ref_no']; ?>"/>
                   ...
                   <button class="fa fa-times" type="submit" name="btnd"></button>
  ...
  echo $row2['stud_fname']."</td><td>".$row2['dept_name']."</td><td>";
       ...
       echo "</td><td>".$row['start_date']." To ".$row['finish_date']."</td><td>".$row['purpose']."</td><td>";
            ...
            <input type="hidden" name="avid" value="1"/>
                   ...
                   <button type="submit" class="fa fa-times" name="btnd"></button>
             ...
             echo "</td></tr></form>"; 
                ...
                if(isset($_POST['btnc']))
                {
                   ...
                   {
                     $vi=$_POST['vid'];
                   $sqlup="update cell set cell_status='Booked' where cell_id=$ci and v_id=$vi";
                   ...
                   if(!$resup)
                    ...
                    {   
                     ...
                     if(!$resl)
                        echo mysql_error();
                         ?> <script type="text/javascript">window.location.assign('http://localhost/vebo/blockadminpanel/production/bookedevents.php');</script>  <?php
                    }
                   }
                   else
                   {
                     $vi=$_POST['aid'];
                    $sqlup="update cell set cell_status='Booked' where cell_id=$ci and a_id=$vi";
                   $resup=mysql_query($sqlup,$con);
                   if(!$resup)
                     echo mysql_error();
                    else
